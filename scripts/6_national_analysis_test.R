...
bcr1 <- subset(bcr1, COUNTRY %in% c("CANADA") | PROVINCE_S == "ALASKA")
...
bcr_temp <- subset(bcr2, region %in% c("West","East") | PROVINCE_S %in% c("ONTARIO","MANITOBA"))
bcr_temp.sp <- as(bcr_temp,"Spatial")
bcr_temp.sp <- gSimplify(bcr_temp.sp, tol = 0.00001)
bcr_temp.sp <- gBuffer(bcr_temp.sp, byid=TRUE, width=0) #further simplify
centroid_union <- unionSpatialPolygons(bcr_temp.sp, bcr_temp$region)

region.centroids <- gCentroid(centroid_union,byid=TRUE)
#--------------------------------------------------------------------
...
centroid_sf <-  st_as_sf(region.centroids, coords = c("lon", "lat"),crs = 4326, agr = "constant")
region_sf <-  st_as_sf(region_union, coords = c("lon", "lat"),crs = 4326, agr = "constant")

region_sf$region <- names(region_union)
centroid_sf$region <- names(region_union)

analysis_site_map <- ggplot() +   theme_bw() +
  geom_sf(data = bcr2, fill = "white", col = "gray95")+
  geom_sf(data = region_sf, aes(color = region), fill = "transparent")+
  
  scale_color_manual(values=col_pal) +
  
  #geom_sf(data = centroid_sf, aes(color = region), size = 5, shape = 19, alpha = 0.5) +
...
central_fall <- data.frame(station = c("PEPBO",
                                       "BPBO",
                                       "PIBO"),
...
analysis_site_map2 <- ggplot() +   theme_bw() +
  geom_sf(data = bcr2, fill = "white", col = "gray95")+
  geom_sf(data = region_sf, aes(color = region), fill = "transparent")+
