...
# sp_path <- ebirdst_download(species = "Vireo bellii")
# 
# # load the abundance data
#abd <- load_raster("abundance_umean", path = sp_path)
abd <- raster("../data/relative_abundance_maps/mosaic-BARS-run3.tif")
...
abd_season_proj <- projectRaster(abd, crs = mollweide, method = "ngb")
...
# mask to land area
ne_land_buffer <- st_buffer(ne_land, dist = max(res(abd_season_proj)) / 2)
pred_region <- mask(abd_season_proj, as_Spatial(ne_land_buffer)) #pred_region <- mask(pred_region, as_Spatial(ne_land_buffer))
...
pal <- abundance_palette(length(bin_breaks$bins) - 1, "breeding")

plot(abd_no_zero, col = pal, breaks = bin_breaks$bins,
     maxpixels = ncell(abd_no_zero),
     legend = FALSE, add = TRUE)
...
title("Blackpoll Warbler relative density (BAM predictions)", 
      line = -1, cex.main = 1)
