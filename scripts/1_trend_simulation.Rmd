...
Assume there are `r nregion` regions (e.g., BCRs), each with a different mean trend (see Figure \ref{fig:fig1}):


```{r fig1, include = TRUE, message = FALSE, fig.cap="\\label{fig:fig1}Trends in each region."}
...
```{r fig2, include = TRUE, message = FALSE, fig.cap="\\label{fig:fig2}Initial abundance in each region."}
...
suppressWarnings(print(N0.plot))
```
...
Assume there is additional temporal variation in trends at each site (i.e., random year effects).  The resulting dynamics in each region over `r T` years are:


```{r fig3, include = TRUE, message = FALSE, fig.cap="\\label{fig:fig3}Population dynamics in each region."}
...
The sum of all regional abundances at each time step is the national abundance.  This is the 
...
for (r in 1:nregion){
  ...
  stations.used <- sample(1:nstation,round(runif(1,1,4))) # Which stations have non-zero probabilities?
...
```{r fig4, include = TRUE, message = FALSE, fig.width = 6, fig.height = 4, fig.cap="\\label{fig:fig4}Migration through each station."}
...
rho.sd <- 0 # Temporal variation in rho (on logit scale)
N.region.station$rho <- plogis(qlogis(N.region.station$rho) + rnorm(nrow(N.region.station),0,rho.sd))
```
...
```{r, include = TRUE, message = FALSE, fig.width = 6, fig.height = 5, fig.cap="\\label{fig:figs}Regional trends"}
...
suppressWarnings(print(N.station.plot))
```
...
2) Composition of birds at each station (catchment data from stable isotopes) in at least one year (in this case, year `r isotope.years`):

```{r, include = TRUE, message = FALSE, fig.width = 6, fig.height = 5}

N.region.station2 <- N.region.station
N.region.station2$N.captured[which(N.region.station2$t %in% isotope.years == FALSE)] <- NA

capture.plot2 <- ggplot() +
  geom_bar(data = subset(N.region.station, t %in% isotope.years == FALSE), aes(x = t, y = N.captured), fill = "gray75", stat = "identity", alpha = 0.7, na.rm = TRUE)+
  
  geom_bar(data = N.region.station2, aes(x = t, y = N.captured, fill = factor(region)), stat = "identity", alpha = 0.7, na.rm = TRUE)+
  scale_fill_manual(values=col.pal, name = "Region")+
...
suppressWarnings(print(capture.plot2))
...
for (s in 1:nstation){
  for (t in isotope.years){
    for (r in 1:nregion){
      N.region.station.array[r,s,t] <- N.region.station[which(N.region.station$region == r & N.region.station$station == s & N.region.station$t == t),]$N.captured
    }
  }
}
...
```{r, include = TRUE, message = FALSE, fig.width = 6, fig.height = 4, eval = FALSE}

#Total continental population dynamics

N.sum$q500 <- apply(out1$sims.list$N.total,2, function(x) quantile(x,0.5))
N.sum$q025 <- apply(out1$sims.list$N.total,2, function(x) quantile(x,0.025))
N.sum$q975 <- apply(out1$sims.list$N.total,2, function(x) quantile(x,0.975))
...
```{r, include = TRUE, message = FALSE, fig.width = 5, fig.height = 3, eval = FALSE}
#Estimate of national trend
trend.est <- apply(log(out1$sims.list$N.total),1, function(x) mean(diff(x)))

hist(trend.est, col = "lightblue", border = "transparent", main = "Trend estimate", ylab = "freq", xlab = "Trend",
     breaks = seq(-0.10, 0.10, length.out = 200))
...
trend$q500 <- apply(out1$sims.list$trend,2,function(x) quantile(x, 0.500))
trend$q025 <- apply(out1$sims.list$trend,2,function(x) quantile(x, 0.025))
trend$q975 <- apply(out1$sims.list$trend,2,function(x) quantile(x, 0.975))
...
```{r, include = TRUE, message = FALSE, fig.width = 6, fig.height = 5, eval = FALSE}

N.df$q500 <- N.df$q025 <- N.df$q975 <- NA

for (i in 1:nrow(N.df)){
  N.df$q500[i] <- quantile(out1$sims.list$N[,N.df$region[i], N.df$t[i]],0.5)
  N.df$q025[i] <- quantile(out1$sims.list$N[,N.df$region[i], N.df$t[i]],0.025)
  N.df$q975[i] <- quantile(out1$sims.list$N[,N.df$region[i], N.df$t[i]],0.975)
}
...
The next section adds additional complexity:

1) Assume only a small number of birds at each station are sampled for isotopes.  This introduces sampling variation into catchment estimates.
2) Assume yearly totals are not directly observed.  Instead, only daily counts are recorded (with error). An observation model is needed to relate daily counts to seasonal totals.
...
N.region.station.array <- array(NA, dim = c(nregion,nstation,T))
for (s in 1:nstation){
  for (y in isotope.years){
    for (r in 1:nregion){
      
      N.region.station.array[,s,y] = subset(N.region.station, t == y & station == s)$N.captured %>%
...
jags.data$N.station.sampled <- jags.data$N.station.total * 0 + N.sampled
```
...
out2 <- jags(data = jags.data,
            model.file = "cmmn1.jags",
            parameters.to.save = c("proc.sd","trend","rho","N.total", "N"),
            inits = inits,
            n.chains = 2,
            n.thin = 25, 
            n.iter = 100000,
...
N.sum$q500 <- apply(out2$sims.list$N.total,2, function(x) quantile(x,0.5))
N.sum$q025 <- apply(out2$sims.list$N.total,2, function(x) quantile(x,0.025))
N.sum$q975 <- apply(out2$sims.list$N.total,2, function(x) quantile(x,0.975))
...
```{r, include = TRUE, message = FALSE, fig.width = 5, fig.height = 3, eval = FALSE}
#Estimate of national trend
trend.est <- apply(log(out2$sims.list$N.total),1, function(x) mean(diff(x)))

hist(trend.est, col = "lightblue", border = "transparent", main = "Trend estimate", ylab = "freq", xlab = "Trend",
     breaks = seq(-0.1, 0.1, length.out = 200))
...
N.daily.df <- expand.grid(t = 1:T, station = 1:nstation, day = 1:nday) %>%
...
Plot of daily catch across 5 years at stations 1 - 3
```{r}

daily.catch.plot <- ggplot( data = subset(N.daily.df, t <= 5 & station <= 3)) +
  geom_line(aes(x = day, y = daily.count))+
  facet_grid(station~t)

print(daily.catch.plot)
```

Incorporate into JAGS model:
...
daily.count.array <- array(NA, dim = c(nday,nstation,T))
for (s in 1:nstation){
  for (y in 1:T){
    for (d in 1:nday){
      
      daily.count.array[d,s,y] = subset(N.daily.df, t == y & station == s & day == d)$daily.count
...
dimnames(jags.data$daily.count)[[1]] <- paste0("d",1:nday)
dimnames(jags.data$daily.count)[[2]] <- paste0("s",1:nstation)
dimnames(jags.data$daily.count)[[3]] <- paste0("t",1:T)

jags.data$N.station.sampled <- N.station.array * 0 + N.sampled
```
