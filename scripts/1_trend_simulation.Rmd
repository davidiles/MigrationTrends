...
setwd("~/1_Work/Research/In Progress/MigrationTrends/scripts")
...
for (r in 1:nregion){
  ...
  rho[stations.used] <- runif(length(stations.used),0.001,0.03) # Insert random probabilities into those stations
...
```{r}
# Years where isotope data were collected
isotope_years <- c(5)
```
...
capture.plot2 <- ggplot() +
  geom_bar(data = subset(N.region.station, t %in% isotope_years == FALSE), aes(x = t, y = N.captured), fill = "gray75", stat = "identity", alpha = 0.7, na.rm = TRUE)+
...
cat("
    ...
    for (r in 1:nregion){
      ...
      logN0.uncert[r] <- logN0[r] #~ dnorm(logN0[r],logN0.tau)    # Uncertainty in initial abundance
    }
...
Estimates of national abundance can be biased
